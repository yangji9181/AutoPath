...
class AutoPath(object):
	...
	def PPO_epoch(self, sess):
		...
		for _ in range(self.params.PPO_step):
	...
	def classification_epoch(self, sess):
		for _ in range(self.params.classification_step):
	...
	def train(self, sess):
		...
		self.average_reward = []
		for _ in tqdm(range(self.params.epoch), ncols=80):
	...
	def plan(self, sess, start_state):
		trials = []
		for _ in tqdm(range(self.params.num_trial), ncols=80):
			states, actions, nexts = self.collect_trajectory(sess, start_state)
